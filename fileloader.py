...
class FileLoader:
    ...
    def load_class(class_path: str, files_per_class: int, num_of_bands=3, is_random=False):
        ...
        :return: An mxn numpy array of samples, a string label
        """
        num_of_features = num_of_bands * 64 * 64
        control_set_x = np.zeros((num_of_features, files_per_class))
        ...
        for i in range(files_per_class):
            with open(file_names[i], 'rb') as file_stream:
                ...
                control_set_x[:, i] = img  # write image vector to the i-th column of X
    ...
    def set_control_set(self, num_of_bands=3, is_random=False):
        ...
        control_set_x = np.empty((self.num_of_features, 0))
        control_set_y = np.empty((1, 0), dtype=np.int8)
        ...
        for class_name in self.classlist:
            ...
            # set up control_set
            control_set_x = np.concatenate((control_set_x, class_set_x), axis=1)
            control_set_y = np.concatenate((control_set_y, np.full((1, self.files_per_class),
                                                                   self.get_class_id(class_name))), axis=1)
    ...
    def __get_train_set(self, samples_per_class):
        ...
        columns_idx = np.random.randint(0, self.files_per_class,
                                        size=samples_per_class)  # list of indices to select from each class
        # create matrix for train set
        (m, n) = (self.num_of_features, len(self.classlist) * samples_per_class)
        train_set_x = np.empty((m, 0))  # TODO: specify dtype for x.
        train_set_y = np.empty((1, 0), dtype=np.int8)
        # col_count = 0
        y_count = 0
        ...
        for x_of_one_class in listy:  # iterate through X of each class
            selected_x = x_of_one_class[:, columns_idx]
            train_set_x = np.concatenate((train_set_x, selected_x), axis=1)
            train_set_y = np.append(train_set_y, np.full((1, samples_per_class), y_count), axis=1)
            y_count += 1
